...
function createMainWindow() {
    ...
    // Load URL
    __mainWindow.loadURL(APP.settings.deezerUrl, {
...
function initializeRPC() {
    RPC.login({ clientId: APP.settings.discordClientID })
        .then(() => {
            setTimeout(Player.registerRPC, 3000);
        })
        .catch(() => {
            dialog.showErrorBox(
                "Rich Presence Login Failed",
                "Please, verify if your discord app is opened/working and relaunch this application.",
            );
        });
}
